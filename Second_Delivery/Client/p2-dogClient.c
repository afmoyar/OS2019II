...
void openEditor(char *path){
    if(!fork()){
        ...
        char ans='N';
        while(ans!='Y'){
            printf("Close records? Dont forget to save (Y/N): \n");
            scanf(" %c",&ans);
            if(ans=='Y'){
            ...
            case 2:;
                ...
                if(success==1)
                {
                    ...
                    if(ans=='Y'){
                        ...
                        close(fd);
                        //sends flag=1 to server to indicate changes were mande
                        flag=1;
                        ...
                        close(fd);
                        //sends flag=-1 to server to indicate no changes were mande
                        flag=-1;
                        r=send(serverfd,&flag, sizeof(flag), 0);
                        checkError(r,-1,"error en el send");
                    }
